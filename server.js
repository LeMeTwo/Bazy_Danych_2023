...
app.post('/Register', async function (req, res) {
	...
	try {
		...
		const selectedMail = await connection.quert('SELECT mail from users where login = \'' + anime.mail + '\';');
...
app.post('/Login', async function (req, res) {
	...
	try {
		...
		const selectedAccount = await connection.query('SELECT login, password, isAdmin from users where login = \'' + anime.login + '\';');
		if (selectedAccount.rows.length) {
			if(selectedAccount.rows[0].password == anime.password){
				if(selectedAccount.rows[0].isAdmin == 1){
